...
export class SyncingComponent extends AppPortalBase
  implements OnInit {
  ...
  syncWorkers() {
    ...
    ref.onClose.subscribe((result: any) => {
      if (result.confirmed) {
        this.settingsRepo.syncServiceWorkers()
...
syncUsers(): void {
  ...
  syncVehicles(): void {
    ...
    ref.onClose.subscribe((result: any) => {
      if (result?.confirmed) {
        this.settingsRepo.syncVehicles()
          .pipe(takeUntil(this.destroyer$))
          .subscribe({
            next: (data: any) => {
              const isSynced = data?.data;
  ...
  getLastSyncTime(): void {
    this.settingsRepo.getLastSyncTime()
      .pipe(takeUntil(this.destroyer$))
      .subscribe({
        next: (data: any) => {
