...
import { Imports } from 'core-ui-salary-calculation-library/src/lib/core/domain/enums/Imports';
...
import { formatDateWithTime } from 'core-ui-salary-calculation-library/src/lib/data/shared/helper.function';
import { ProgressLoadingComponent } from "../../shared/progress-loading/progress-loading.component";


@Component({
  selector: 'lib-syncing',
  imports: [CommonModule, CheckboxModule, FileUploadModule, BreadcrumbModule, TabViewModule, AccordionModule, FormsModule, ReactiveFormsModule, DeadlinePeriodsComponent, TabsModule, ButtonModule, ProgressLoadingComponent],
  templateUrl: './syncing.component.html'
})
export class SyncingComponent extends SalaryCalculationPortalBase
  implements OnInit {
  ...
  constructor(inject: Injector, private dataNextService: DataNextService,
  ...
  syncWorkers() {
    ...
    ref.onClose.subscribe((result: any) => {
      if (result?.confirmed) {
        this.importService.SyncServiceWorkers()
            ...
            next: (data: any) => {
              ...
              if (isSynced) {
                ...
                this.messageService.add({
                  ...
                  detail: this.translate.instant('ELIGA_WORKERS_SYNCED_SUCCESSFULLY'),
                });
                this.getLastSyncTime();
              } else {
                this.messageService.clear();
...
syncUsers(): void {
  ...
  syncVehicles(): void {
    ...
    ref.onClose.subscribe((result: any) => {
      if (result?.confirmed) {
        this.importService.SyncVehicles()
          .pipe(takeUntil(this.destroyer$))
          .subscribe({
            next: (data: any) => {
              const isSynced = data?.data;
              if (isSynced) {
                ...
                this.messageService.add({
                  ...
                  detail: this.translate.instant('ELIGA_VEHICLES_SYNCED_SUCCESSFULLY'),
                });
                this.getLastSyncTime();
              } else {
                this.messageService.clear();
  ...
  getLastSyncTime(): void {
    this.importService.getLastSyncTime()
      .pipe(takeUntil(this.destroyer$))
      .subscribe({
        next: (data: any) => {
          this.assignSyncing(data?.data);
        },
        error: (err: any) => {
          this.messageService.clear();
          this.messageService.add({
