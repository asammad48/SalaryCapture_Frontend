  ...
  implements OnInit, OnDestroy
{
  ...
  UpdateUserModal(user: User): void {
    ...
    ref.onClose.subscribe(async (result: ApiResponse<User>) => {
      if (result?.success) {
        try {
          await lastValueFrom(
            this.UserRepository.updateUser(result.data).pipe(
  ...
  deleteUserModal(user: User): void {
    ...
    ref.onClose.subscribe(async (confirmed: boolean) => {
      ...
      try {
        await lastValueFrom(
          this.UserRepository.deleteUser(user.id).pipe(
            takeUntil(this.destroyer$)
          )
        );
  ...
  disableUserModal(user: User): void {
    ...
    ref.onClose.subscribe(async (confirmed: boolean) => {
      ...
      try {
        await lastValueFrom(
          this.UserRepository.disableUser(user).pipe(takeUntil(this.destroyer$))
        );
        this.getUsers();
