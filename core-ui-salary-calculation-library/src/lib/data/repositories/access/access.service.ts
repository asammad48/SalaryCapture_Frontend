...
import { LocalStorageService } from "core-ui-salary-calculation-library/src/lib/presentation/services/local-storage.service";
import { PermissionsType } from "core-ui-salary-calculation-library/src/lib/core/domain/constants/claims.constants";
import { LocalStorageKeys } from "./local-storage-keys";
import { TenantConfigurationService } from "core-ui-salary-calculation-library/src/lib/presentation/services/tenant-configuration.service";
...
export class AccessService {
  ...
  getUserAreasRoles(userId: string): Observable<ApiResponse<CurrentUserAreaRoles[]>> {
    return this.http.get<ApiResponse<CurrentUserAreaRoles[]>>(
      `${process.env["NX_BASE_DP_URL"]}${AccessApiUrl.CurrentUserAreasRoles}/${userId}`
    );
  }

  getCurrentUserSelectedRegionAccess(regionId: string): Observable<ApiResponse<boolean>> {
    return this.http.get<ApiResponse<boolean>>(
      `${process.env["NX_BASE_DP_URL"]}${AccessApiUrl.GetCurrentUserSelectedRegionAccess}/${regionId}`
    );
  }
